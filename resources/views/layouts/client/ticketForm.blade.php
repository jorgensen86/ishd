                ...
                <form action="{{ route('client.ticket.store') }}" method="post" id="ticketForm">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                @csrf
                                <x-form.select selected="" label="{{ __('ticket.domain') }}" name="domain_id" :options="$invoices"></x-form.select>
                                <x-form.select selected="" label="{{ __('ticket.group') }}" name="subject_id" :options="$subjects"></x-form.select>
                                <x-form.text name="subject" id="inputSubject" placeholder="{{ __('ticket.subject') }}" value=""></x-form.text>
                                <x-form.ckeditor label="{{ __('ticket.message') }}"  :id="'bodyEditor'" :name="'body'"></x-form.ckeditor>
                            </div>
                            {{--/ Ticket Form  --}}
